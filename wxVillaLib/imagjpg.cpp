...
CPP_METHODDEF(void) wx_init_source1 ( j_decompress_ptr WXUNUSED(cinfo) )
{
}

CPP_METHODDEF(wxjpeg_boolean) wx_fill_input_buffer1 ( j_decompress_ptr cinfo )
{
...
CPP_METHODDEF(void) wx_skip_input_data1 ( j_decompress_ptr cinfo, long num_bytes )
{
    if (num_bytes > 0)
    {
        wx_src_ptr src = (wx_src_ptr) cinfo->src;
...
CPP_METHODDEF(void) wx_term_source1 ( j_decompress_ptr cinfo )
{
    wx_src_ptr src = (wx_src_ptr) cinfo->src;
...
CPP_METHODDEF(void) wx_error_exit1 (j_common_ptr cinfo)
{
 ...
 */
CPP_METHODDEF(void) wx_ignore_message1 (j_common_ptr WXUNUSED(cinfo))
{
}
...
void wx_jpeg_io_src( j_decompress_ptr cinfo, wxInputStream& infile )
{
    ...
    src->pub.init_source = wx_init_source1;
    src->pub.fill_input_buffer = wx_fill_input_buffer1;
    src->pub.skip_input_data = wx_skip_input_data1;
    src->pub.resync_to_restart = jpeg_resync_to_restart; /* use default method */
    src->pub.term_source = wx_term_source1;
}
...
bool wxJPGHandler::LoadFile( wxImage *image, wxInputStream& stream, bool verbose, int WXUNUSED(index) )
{
    ...
    const unsigned maxWidth = image->GetOptionInt(wxT("max_width")),
                   maxHeight = image->GetOptionInt(wxT("max_height"));
    image->Destroy();

    cinfo.err = jpeg_std_error( &jerr );
    jerr.error_exit = wx_error_exit1;

    if (!verbose)
        cinfo.err->output_message = wx_ignore_message1;
...
CPP_METHODDEF(void) wx_init_destination1 (j_compress_ptr cinfo)
{
...
CPP_METHODDEF(wxjpeg_boolean) wx_empty_output_buffer1 (j_compress_ptr cinfo)
{
    wx_dest_ptr dest = (wx_dest_ptr) cinfo->dest;
...
CPP_METHODDEF(void) wx_term_destination1 (j_compress_ptr cinfo)
{
    wx_dest_ptr dest = (wx_dest_ptr) cinfo->dest;
...
GLOBAL(void) wx_jpeg_io_dest (j_compress_ptr cinfo, wxOutputStream& outfile)
{
    ...
    dest->pub.init_destination = wx_init_destination1;
    dest->pub.empty_output_buffer = wx_empty_output_buffer1;
    dest->pub.term_destination = wx_term_destination1;
    dest->stream = &outfile;
}
